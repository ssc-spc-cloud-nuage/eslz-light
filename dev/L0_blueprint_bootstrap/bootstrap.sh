...
if [[ ! -f "./backend.azurerm.tf" ]]; then
  ...
  if [ $RETURN_CODE != 0 ]; then
    ...
    echo "INFO - generating remote state configuration ${TF_VAR_env_folder_name}.yaml"
    cat << EOF > ../${TF_VAR_env_folder_name}.yaml
remote_state:
  tenant_id: "${tenant_id}"
...
EOF
    echo "INFO - Do not forget to check-in ${TF_VAR_env_folder_name}.yaml to source code management"

    echo "INFO - generating envvars file ${TF_VAR_env_folder_name}.envvars"
    cat << EOF > ../${TF_VAR_env_folder_name}.envvars
tenant_id="${tenant_id}"
subscription_id="${subscription_id}"
resource_group_name=${resource_group_name}
storage_account_name=${storage_account_name}
...
EOF
    echo "INFO - Do not forget to check-in ${TF_VAR_env_folder_name}.envvars to source code management"
  fi
else
  terragrunt init  
  terragrunt apply -var-file=deploy.tfvars
