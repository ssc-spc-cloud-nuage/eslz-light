...
data "azuread_service_principal" "L1_Subscription_Owners-SP_by_names" {
  for_each = toset(var.L1_RBAC.ownerSPNames)

  display_name = each.key
}

resource "azuread_group_member" "L1_Subscription_Owners-Members_by_objectIDs" {
  for_each = data.azuread_service_principal.L1_Subscription_Owners-SP_by_names
